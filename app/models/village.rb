...
class Village < ApplicationRecord
  ...
  def related_plans
    Plan.where(planable_type: 'Village', planable_id: self.id)
          .or(Plan.where(planable_type: 'Facility', planable_id: facility_ids))
  end

  def related_reports
    Report.where(reportable_type: 'Village', reportable_id: self.id)
